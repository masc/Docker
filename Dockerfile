...
RUN pip3 install --user pylint radon codecov nose \
&& pip3 install -r https://raw.githubusercontent.com/socialcars/colmto/master/requirements.txt --user
...
&& sudo /usr/local/texlive/bin/x86_64-linux/tlmgr update --self --all --reinstall-forcibly-removed
